function [y_sollb,dy_sollb,sol_lb,y_solub,dy_solub,sol_ub,...
    ...
    = nsligfp_bootstrap(x_data,y_data,dy_data,sol,y_sol,dy_sol,...
    len_sol,imposeconstr,chngsolver,nboot,...
...
boot_sol = bootstrp(nboot, ...
    @(bootry,bootrdy)bootregs(y_est + bootry, dy_est + bootrdy,...
    y_mdlfun,dy_dx_mdlfun,x0,nlsigprob,...
    imposeconstr,chngsolver,newoptins,nboot,skyblue),...
    fitstats.residuals,fitstats.dresiduals);    
pause('off');clear bootregs;
