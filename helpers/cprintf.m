...
function [underlineFlag,boldFlag,style,debugFlag] = processStyleInfo(style)
  ...
  % fix: by oasomefun@futa.edu.ng, 2020
  % ensures bold format for matlab's RGB colour vector
  % usecase: cprintf("*[1, 0, 0]",'bold red\n')
  % cprintf("*[0.75, 0.75, 0.75]','bold grey\n')
  if isstring(style) && ~boldFlag && contains(style,"*")
      style = erase(style,"*");
      boldFlag = 1;
      style = str2num(style); %#ok<ST2NM>
  end
  ...
  % removed && all(style<=1)
  if isnumeric(style) && length(style)==3  && all(abs(style)>=0)
      if any(style<0)
          underlineFlag = 1;
          style = abs(style);
      end
